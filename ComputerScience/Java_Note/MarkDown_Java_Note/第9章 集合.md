...
可以更简洁地将这个循环写为"for each"循环访问该集合，
...
​	Map又被叫做双列集合。存储的数据叫做键值对（Entry对象）。

​								 <img src="../IMG/image-20260213121523245.png" alt="image-20260213121523245" style="zoom:50%;" />	

​	

​	Map是双列集合的顶层接口，定义在该接口中的方法，实现类均可使用
...
/*
	现在映射的框架大概是这样： var counts = new HashMap<String,Integer>();
	一个单词（键）对应一个计数器（值）
...
default V merge(K key, V value,
    ...
    接口参数第一个V对应旧值，第二个v对应新值，第三个v对应返回值
    如果key与一个非null值v关联，将函数（这里指Bifunction接口中的函数接口）应用到v和value，将key与结果关联，或者如果结果为null，则删除这个键，否则，将key与value关联，返回 get(key);
	
default V compute(K key, BiFunction<? super K,? super V ? extends V> remappingFucntion)
    将函数应用到key和get(key)。将key与结果关联，或者如果结果为null，则删除这个键，返回get(key)；简单来，通过Key的旧值计算新值，并将新值与key关联
	接口参数第一个对应key，第二个对应key的value，第三个对应返回值
    
    
default V computeIfPresent(K key, BiFunction<? super V,? super V ? extends V> remappingFucntion)
   接口参数对应关系与上面一致;
    如果key与一个非null值v关联，将函数应用到key和v，将key与结果关联，或者结果为null，则删除这个键，返回get(key);简单来说，同样是通过key的旧值更新新值，不过前提是key的值不能为空。
	解释2：
    当 key 存在且映射到非 null 值时，才执行计算，用新值替换（如果新值为 null 则删除 key）。反之如果 key 不存在或值为 null，不执行函数，返回 null。
...
default V computeIfAbsent(K key,Function<? super K,? extends V> mappingFunction)
	...
	解释2：
    如果 key 当前不存在或映射为 null，则调用 mappingFunction 计算值，并将结果与 key 关联（如果计算值非 null）
...
default V putIfAbsent(Key,V value)
       如果map中key不存在，或者key对应的value是null就放进去。
```

`marge`方法解析：

- **`key`**：要操作的键。
- **`value`**：当键不存在（或原值为 `null`）时，用作初始值的“种子”值。它也会作为合并函数的一个参数。
- **`remappingFunction`**：一个 `BiFunction`，接受两个参数：
  - 第一个参数：当前键关联的旧值（非 `null`）。
  - 第二个参数：传入的 `value`（即上述种子值）。
  - 返回值：计算得到的新值。如果新值为 `null`，则最终会从映射中移除该键。
...
#### 9.4.5 链接散列集与映射。LinkedHashMap

​	相比 HashMap，LinkedHashSet和LinkedHashMap类会记住插入元素项的顺序，所以可以使用迭代器来访问。它键的特点有：

​	**有序，不重复，无索引**

​	这里的有序会保证存储和取出的元素顺序一致。底层数据结构依然是哈希表，只是每个键值对元素又额外的多了一个双链表的机制记录存储的顺序。

```java
        //创建一个链接散列映射
        LinkedHashMap<String,Employee()> staff = new LinkedHashMap<String,Employee>();
...
```java
  public static void main(String[] args) {
        ...
        //获取所有的键，放到一个单列集合中，keySet会返回一个集合，存储该map的所有键
...
​	所有的单列集合，即继承自`Collection`接口的实现类，都支持迭代器访问，因为在该接口中，就定义了迭代器成员变量。在上一节程序的基础上，通过迭代器访问得到的单列集合
...
```java
 public static void main(String[] args) {
        ...
        System.out.println("使用lambda表达式访问Map");
        //使用lambda表达式
...
​	HashMap是Map接口的实现类。对于HashMap的键，下面几个特点：
...
- **无索引**
  ...
  HashMap跟HashSet底层原理是一模一样的，都是**哈希表结构 **

​	HashMap在存储时，遇见哈希值相同的键，会挂在原来的键上，形成链表

![](../IMG/image-20260213134915011.png)

​	当链表长度超过8&&数组长度>=64，自动转成红黑树。注意，**HashMap只保证键的唯一，不保证值的唯一**。

​	它以来`hashCode`方法和`equals`方法保证键的唯一。**如果键存储的是自定义对象，需要重写`hashCode`和`eqauls`方法**
...
​	TreeMap跟TreeSet底层原理一样，都是红黑树结构。特性有：**不重复、无索引、可排序**。默认按照键的从小到大进行排序，也可以自己规定键的排序规则。



​	**TreeMap集合的排序是根据传入键的类实现`Comparable`接口，来指定比较规则。**还可以在创建集合时传递`Comparator`比较器对象指定规则。**如果键实现了接口的同时，创建集合还传递了比较器对象，那么则按照比较器对象的规则来排序。**

​	总结来说，TreeMap指定比较器对象有两种方式：

1. 在构造时传入比较器`Comparator`对象。
...
/**
	题目2：
		键：学生对象
		值：籍贯（String）
		要求: 按照学生年龄的升序排列，年龄一样的按照姓名的字母排列，同姓名年龄视为同一个人
*/
...
​	对于有序集和有序映射，可以使用排序顺序而不是元素位置建立子范围。`SoredSet`接口声明了3个方法：

```java
//从1开始
SortedSet<E> subSet(E from, E to) //返回此集合中元素范围从`from`（包括）到`to`（不包括）的部分的视图
SortedSet<E> headSet(E to)//返回此集合中元素小于`to`的部分的视图。
SortedSet<E> tailSet(E from)// 返回此集合中元素大于等于`from`的部分的视图。
```
...
​	这些方法会返回映射的视图，其中包含落在指定范围内的键相应的所有元素。注意，Map本身并不保证有序性，但它的子接口`SortedMap`和`NavigableMap`保证了有序性，`hashMap`并没有这些方法

​	Java6引入的`navigableSet`接口允许对这些子范围操作有更多的控制。可以指定是否包括边界：

```java
NavigableSet<E> subSet(E from,boolean fromInclusive,E to,boolean toInclusive);
NavigableSet<E> subSet(E to,boolean toInclusive);
...
| public static  \<T\> void sort(List \<T\> list , Comparator \<T\> c) | 根据指定的规则排序                 |
