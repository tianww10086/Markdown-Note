			xxxxxxxxxx1 1本章将介绍Java中用于输入和输出的各种应用编程接口（API）。将学习如何访问文件和目录，以及如何以二进制格式和文本格式来读写数据。还要学习对象序列化机制，它可以使存储对象像存储文本和数值数据一样容易。然后将介绍如何使用文件和目录，最后本章节会讨论正则表达式（它处理字符串很有用）
