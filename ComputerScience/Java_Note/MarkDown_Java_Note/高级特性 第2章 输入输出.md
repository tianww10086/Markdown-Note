...
​	序列号机制提供了一种有趣的用法：**即提供了一种克隆对象的简便途径，只要对应的类是可序列号的即可。**其做法是：

​	**直接将对象序列号到输出流中，然后将其读回。这样产生的新对象是对现有对象的一个深拷贝。在此过程中，不必将对象写出到文件中，因为可以用`BetyArrayOutputStream`将数据保存到字节数组中**
...
​	但需要注意，所有成员都必须`implements Serializable ` ，这里的`LocalDate`已经实现
...
```java
Path workPath = basePath.reslove("work");
```
...
Stream<String> lineStream = Files.lines(path,charset);//读入所有行，存入stream流
```
...
​	下面的方法使你可以使用经典的输入/输出流或者读入器/写出器的API进行交互:

```java
InputStream in = Files.newInputStream(path); //字节流输入
OutputStream out = Files.newOutputStream(path); //字节流输出
Reader in = Files.newBufferedReader(path,charset); //带缓冲区的字符流输入
Write out = Files.newBufferedWriter(Path,charset);//带缓冲区的字符流输出
```

​	这些便捷方法可以将你从处理`FileInputStream、FileOutputStream、BufferedReader和BuferedWrite`的繁复操作中解脱出来。
...
static Path writeString(Path path,CharSequence contents,Charset charset,OpenOption... options)
static Path write(Path path,Iterable<? extends CharSequence> contents,Charset charset,OpenOption...options)
    将给定内容写出得到文件中，并返回path
static long mismatch(Path path,Path path2)
	返回两个文件的内容中第一个不相同的字节的位置，从0开始
static String probeContentType(Path path)
    返回针对文件内容的MIME类型的各种已安装的文件类型探测器中的最佳匹配，或者在无法确定类型时返回null。
...
​	创建新目录可以调用下面这个方法，该方法要求除了路径中最后一个部件外，其他部分都必须存在。

```java
Files.createDirecotry(path);
```

​	比如，如果路径是`C:/data/report/newDir`，那么`C:/data/report`必须存在
...
​	如果文件已经存在，上面的调用就会抛出异常，如果文件不存在，该文件就会被创建。**检查文件是否存在和创建文件是原子性的**。

​	下面的便捷方法啊可以用来在给定位置或系统指定位置创建临时文件或临时目录：

```java
Path newPath =  Files.createTempFile(dir,prefix,suffix);
Path newPath =  Files.createTempFile(prefix,suffix);
Path newPath =  Files.createTempDirectory(dir,prefix);
Path newPath =  Files.createTempDirectory(preix);
```
...
​	如果想要覆盖已有的目标路径，可以使用`REPLACE_EXISTING`选项。如果想要复制所有的文件属性，可以使用`COPY_ATTRIBUTES`选项。可以同时选择这两个选项：
...
​	选项`ATOMIC_MOVE`可以保证移动操作的原子性，要么移动完成，要么源文件继续保持在原来位置。

```java
Files.move(fromPath,toPath,StandardCopyOption.ATOMIC_MOVE);
```

​	`ATOMIC_MOVE` 要求**源路径和目标路径必须在同一个文件系统卷（比如同一个硬盘分区）内**。跨卷的移动无法保证原子性，此时即使你加了该选项，也会抛出 **`AtomicMoveNotSupportedException`**
...
​	如果删除文件不存在，这个方法会抛出异常，因此，可转而使用下面的方法：
...
static Path move(Path from,Path to,CopyOption... option)
...
static long copy(Path from,OutputStream to ,CopyOption... options)
    从输入流复制到文件中，或者从文件复制到输出流中，返回复制到字节数。
...
​	`getOnwer`方法将文件的拥有者作为`java.nio.file.attribute.UserPrincipal`的一个实例返回：

```java
        //获取输入流
        Path filePath = Path.of("E:\\单片机实例\\第二次实例\\Code\\16X16LED点阵显示屏.hex");
...
- 文件是常规文件、目录还是符号链接，亦或者三者都不是
- 文件尺寸
- 文件主键，这是某种类的对象，具体所属类于文件系统相关，有可能是文件的唯一标识符，也可能不是
...
​	然后可以从中找到组拥有者，以及文件的拥有者、组合访问权限。但是这种信息中的很多内容在操作系统之间并不具备可移植性
...
```java
static boostatic boolean exists(Path path)
...
​	静态的`File.list`方法会返回一个读取目录中的各个项的`Stream<Path>`对象，目录是被惰性读取的，这使得处理具有大量项的项目可以变得更高效，
...
​	list方法不会进入子目录。为了处理目录中的所有子目录，应该使用File.walk方法：
...
​	这段代码使用了`Files.walk`方法来讲一个目录复制到另一个目录：

```java
public class Test_File {
    public static void main(String[] args) throws IOException {
        Path dir1 = Path.of("E:\\Code\\C Program\\数组");
        Path dir2 = Path.of("E:\\Code\\Dev C++_Project");
        //把目录1复制到目录2去
        Files.walk(dir1).forEach(p->{
            //p代表dir1里的每个目录或文件
            try{
                /*E:\Code\C Program\\数组
                relativize
                 E:\Code\C Program\\数组\\ArrTest\app.ico
                结果： ArrTest\app.ico

                然后把这个结果拼接到目标路径上
                dir.resolve(dir1.relativize(p));
                等于= E:\Code\Dev C++_Project\ArrTest\app.ico
                */

                Path q = dir2.resolve(dir1.relativize(p));
...
​	现在还无法容易的使用`Files.walk`方法来删除目录数，因为你必须在删除父目录之前先删除子目录。下一节介绍如何克服这个问题。



#### 2.4.7 使用目录流

​	前一节中， 使用`Files.walk`方法会产生一个可以遍历目录中所有子孙的`Stream<Path>`对象。

​	在需要对遍历过程进行更加细粒度的控制时，应该使用`Files.newDirectoryStream`对象，它会产生一个`DirectoryStream`。注意，它不是`java.util.stream.Stream`的子接口，而是专门用于目录遍历的接口。**它是`Iterable`的子接口**，因此可以在增强for循环中使用目录流。
...
​	`Glob`模式是一种用于匹配文件路径和文件名的通配符语法，它比正则表达式更简单、直观。
...
| **    | 匹配跨目录边界的0个或多个字母                       | **.java匹配在所有子目录中的Java文件                      |
| ？    | 匹配一个字符                                        | ????.java匹配所有四个字符的Java文件（不包括扩展名）      |
| [...] | 匹配一个字符集合，可以使用连线符[0-9]和取反符![0-9] | Test[0-9A-F].java匹配Testx.java，其中x是一个十六进制数字 |
| {...} | 匹配由逗号隔开的多个可选项之一                      | *.{java,class}，匹配所有的java文件和类文件               |
| \     | 转义上诉任意模式中的字符以及\字符                   | *\\** 匹配文件名中包含\*的文件                           |



​	如果想要访问某个目录的所有子孙成员，可以调用`walkFileTree`方法，并向其传递一个`FileVistor`类型的对象。`FileVistor`是一个接口，它有四个抽象回调方法,`walkFileTree`方法会在下列时刻通知对应的方法:
...
​	对于上诉每种情况，都可以指定是否希望执行下面的操作：

- 继续访问下一个文件：`FileVisitResult.CONTINUE`

- 继续访问，但是不再访问这个目录下的任何项：`FileVisitResult.SKIP_SUBTREE`

- 继续访问，但是不再访问这个文件的兄弟文件（即同级别的文件）了；`FileVisitResult.SKIP_SIBLINGS`;

- 终止访问：`FileVisitResut.TERMINATE`。

  当任何方法抛出异常时，就会终止访问，而这个异常会从`walkFileTree`方法中抛出



​	便捷类`SimpleFileVisitor`实现了`FileVisitor`接口，但是其除`visitFileFailed`方法之外的所有方法并不做任何处理，而是直接继续访问。`visitFileFailed`方法会抛出由失败导致的异常，并而终止访问：

​	下面的代码展示了如何打印给定目录下的所有子目录：

```java
 Path path = Path.of("E:\\Markdown Note");
        Files.walkFileTree(path,new SimpleFileVisitor<>(){
            ...
            public FileVisitResult visitFileFailed(Path file, IOException exc) throws IOException {
                return FileVisitResult.SKIP_SUBTREE;//继续访问，但是不再访问这个目录下的任何项：
            }
        });
    }
```

​		注意，需要覆盖`postVisitDirectory`方法和`visitFileFailed`方法，否则，访问会在遇到不允许打开的目录或不允许访问的文件时立即失败。

​		还应该注意的是，路径的众多属性是作为`preVisitDirectory`和`visitFile`方法的参数（`BasicFileAttributes`）传递的，这样避免了系统调用获取属性这样耗时相对昂贵的操作，直接使用传入的`attrs`参数即可获取文件信息。

​	

​	如果你需要在进入或离开一个目录时执行某些操作，就可以使用`FileVisitor`接口中的方法就很有用。例如，在删除目录树时，需要在移除当前目录的所有文件之后，才能移除该目录，下面是删除目录树的完整代码：

```java
Files.walkFileTree(root,new SimpleFileVisitor<Path>(){
    public FileVisitResult visitFile(Path file,BasicFileAttributes attrs) throws IOException
    {
        Files.delete(file);//删除这个文件或目录
