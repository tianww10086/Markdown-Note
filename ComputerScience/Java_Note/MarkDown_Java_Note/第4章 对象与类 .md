...
​	一个类可以使用所属包中的所有类，以及其他包中的公有类。我们可以采用两种方式访问另一个包中的公有类。第一种方式是在每个类名之前添加完整的包名，即**完全限定名（完整的包名后面跟着类名）**。例如：
...
```java
Date day; //ERROR -- java.util.Data or java.sql.Date
```

​	此时编译器不知道你想用哪个包里面的类。如果两个类都需要使用，那就每个类名加上完整的包名。
...
​	在包中定位类是编译器（compiler）的工作。类文件的字节码总是使用完整的包名来引用其他类。
...
import语句不仅可以导入类，**还增加了导入静态方法和静态字段的功能**，比如：
...
​	将源文件放到与完整包名匹配的子目录中。例如，`com.horstman.corejava`包中的所有源文件放置在子目录`com\horstmann\corejava`中。编译器也将类文件放在相同的目录结构中。

​	如果一个类属于一个包，那么这个.java文件就必须在子目录包下。比如，`Employee`类属于`com.horstmann.corejava`包，因此，`Employee.java`文件就必须在子目录`com\horstmann\corejava`下。比如，有这么一个结构的项目（PackageTest属于无名包）：

![image-20260411212008586](../IMG/image-20260411212008586.png)

​	想要编译这个程序，只需切换到基目录。并运行下面的命令：

```cmd
javac PackageTest.java
```

​	编译器就会自动的查找文件`com/horstmann/corejava/Employee.java`进行编译。这是因为,`PackageTest.java`中使用了`com.horstmann.corejava.Employee`。它会自动并查找那个类

​	

#### 4.8.4 包访问权限

​	包访问是Java中一种权限。我们知道，标记为`public`的部分可以由任意类使用。标记为`private`的部分只能由定义它们的类使用。而如果没有指定任何权限标识符，这个部分（类、方法或变量）可以由同一个包中的所有类访问。
...
​	类存储在文件的子目录中，而类的路径必须与包名匹配。

​	类文件也可以存储在`JAR(java归档)`文件中。在一个`JAR`文件中，可以包含多个压缩格式的类文件和子目录。当程序中用当第三方的库时，通常会得到一个或多个这样的JAR包。

​	在之前，我们使用的都是无名包或者官方自带的API，所以使用是不需要指定类路径的。但是，当你的类中，使用了第三方库或者自定义的类时，而且这个类还在别的文件夹中，你就需要指定这个类的类路径。

​	虽然现在这些工作已经被IDEA和Maven自动化了，但还是需要理解类路径是什么东西。**用大白话来说，就是设置这个类的文件地址，好让JVM能找到这个类并运行**

​	注意，使用命令设置的类路径，仅对当前窗口有效。

​	如果想要让一个类使用第三方类，我们做以下几点：

1. 将第三方类放入到目录中。如果是`Programm/classdir`目录，且第三方类是`com.horstmann.corejava.`的Employe.java，那么`Employee.class`类文件就必须位于：`Programm/classdir/com.hortmann.corejava`中。
2. 将`jar`文件放置在一个目录中。
