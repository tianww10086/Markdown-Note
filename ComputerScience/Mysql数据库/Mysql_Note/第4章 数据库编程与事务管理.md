...
​	前面提到过，索引是分为两种存储方式。一种是`B+tree`，一种是`Hash`。这是Innodb存储引擎的实现，其他存储引擎的存储实现各不同，下面介绍这些索引类型的详细特征：

- `B+Tree`：最常见的索引类型，大部分引擎都支持B+树索引
- `Hash`：底层数据结构是由哈希表实现的，只有精确匹配索引列的查询才有效，不支持范围查询
- `R-tree(空间索引)`: 空间索引是MyISAM引擎的一个特殊索引类型，主要用于地理空间数据类型
- `Full-Text`: 是一种通过建立倒排索引，快速匹配文档的特殊方式。



##### 1. B-Tree

​	在介绍B+Tree之前先介绍B树，B+树是B树的变体，B+树是Mysql底层最核心的数据结构，专门为磁盘存储和大规模数据范围查询量身定制。

​	

​	B树（多路平衡查找树）是一种查找树，但不是二叉树结构。

​	以一个课最大度数为5阶的b-tree为例 （每个节点最多存储4个key，5个指针）

​	![image-20260812133559958](../IMG/image-20260812133559958.png)
...
​	一般的MySQL语句都是直接针对数据库表执行和编写的，这叫做*隐含提交（implicit commit）*，即提交操作是自动进行的。
...
​	在这个例子中，从表中完全删除订单20010，因为涉及更新两个表，所以使用事务处理来保证订单不被部分删除（即一个删除了另一个还存在）。最后的`COMMIT`语句仅在不出错时写出更改。如果第一条DELETE其作用，但第二条失败，则DELETE不会提交（实际上，他是被自动撤销的）。

​	

**3. 使用保留点（SAVEPOINT）**

​	简单的`ROLLBACK`和`COMMIT`语句可以写入或撤销整个事务处理。但对于复杂的事务可能需要部分提交或回退。

​	为了支持回退部分事务处理，必须能在事务处理块中合适的位置放置占位符，这些占位符称为保留点，使用如下关键字创建占位符：

```sql
SAVEPOINT delete1;
```

​	每个保留点都取标识它的唯一名字。以便知道在回退时回退到何处。然后使用`ROLLBACK TO delete1`;
...
​	默认的MySQL是自动提交所有更改。为指示MySQL不自动提交更改，需要使用以下语句：

```sql
SET autocommit=0;
```

​	`autocommit`标志决定是否自动提交更改，不管有没有`COMMIT`语句，设置autocommit为0让它不自动提交。
