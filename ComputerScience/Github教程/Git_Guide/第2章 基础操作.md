...
​	特性分支，是指集中实现单一特性（主体），除此之外不进行任何作业的分支。在开发中，往往会创建数个特性分支，同时在此之外。再保留一个可以随时发布软件的稳定分支。稳定分支的角色通常由`master`分支承担

​						![image-20260711003647227](../IMG/image-20260711003647227.png)

​	在演示中，创建了`feature-A`分支，这一分支主要实现`feature-A`。除此之外，该分支不再进行任何作业，即使在开发的过程中发现了BUG，也需要再创建新的分支，在新分支中进行修正。

​	基于特定主体的作业在特性分支中进行，主题完成后再与`master`分支合并，只要保持这样一个开发流程，就能保证`master`分支可以随时供人查看。其他开发者也可以放心大胆地从`master`分支创建新的特性分支



#### 2.2.5 主干分支

​	主干分支是刚才我们讲解的特性分支的原点。同时也是合并的重点。通常会使用`master`分支作为主干分支。

​	有时候需要让这个主干分支总是配置在正式环境中，有时又需要标签Tag等创建版本信息，同时管理多个版本发布。永远多个版本发布时，主干分支也有多个





#### 2.2.6 git merge -- 合并分支

​	假设分支A已经实现完毕，想要将他合并到主干分支master中，首先切换到master分支

```bash
git checkout master
```

​	然后合并分支A，为了在历史记录中明确记录本次分支合并，需要创建合并提交。因此，在合并时，加上`--no-ff`参数
...
```bash
Marge branch 'feature-A'

....一些默认信息
```

​	将编辑器中显示的内容保存，关闭编辑器，然后就会看见下面的结果

![image-20260711004613297](../IMG/image-20260711004613297.png)

​	这样，分支A就合并到了master分支了

​	`git log --graph`可以以树形结构查看日志。可以很清楚地看见每次的提交，包括本节将的分支合并的内容
...
#### 2.3.1 git reset 回溯历史版本

​	![image-20260811100013087](../IMG/image-20260811100013087.png)

​		现在要回溯到`feature-A`之前，去创建新的分支`fix-b`。要让仓库的`HEAD、暂存区、当前工作树`回溯到指定状态，要用到

```bash
git rest --hard 哈希值
```

​		只要提供目标时间的哈希值（使用`log`查看），就可以完全恢复至该时间点的状态。
